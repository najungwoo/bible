...
def select_day(index):
    ...
    if 0 <= index < len(original_scriptures):
        day_num = index + 1
        scripture.clear()
        scripture.extend(original_scriptures[index])
        left_verse = len(scripture)
        reload_texts()
...
def open_add_verse_popup():
    win = tk.Toplevel(root)
    win.title("구절 추가")
    win.geometry("400x400")
    
    tk.Label(win, text="장절 (예: (요 3:16))").pack(pady=(10, 0))
    ref_entry = tk.Entry(win, width=40)
    ref_entry.pack(pady=5)
    
    tk.Label(win, text="구절 내용").pack(pady=(10, 0))
    verse_text = tk.Text(win, height=5, width=40)
    verse_text.pack(pady=5)
    
    tk.Label(win, text="저장할 파일 (예: day1.txt)").pack(pady=(10, 0))
    
    # 파일 선택 콤보박스
    file_var = tk.StringVar()
    file_cb = ttk.Combobox(win, textvariable=file_var, width=37)
    file_cb['values'] = original_filenames
    ...
    def save_verse():
        ...
        content = verse_text.get("1.0", tk.END).strip()
        fname = file_var.get().strip()
        
        if not ref or not content or not fname:
        ...
        if not (ref.startswith("(") and ref.endswith(")")):
             messagebox.showwarning("경고", "장절은 괄호로 묶여야 합니다. 예: (요 3:16)")
             return
        ...
        try:
            # 파일이 없으면 생성, 있으면 추가
            # 파일이 존재하고 마지막 줄이 개행문자로 끝나지 않으면 개행 추가
            need_newline = False
            if os.path.exists(p):
                with open(p, "rb") as f:
                    try:
            ...
            messagebox.showinfo("성공", "구절이 추가되었습니다.")
            
            # 데이터 재로딩
            global original_scriptures, original_filenames
            original_scriptures, original_filenames = load_original_scriptures_txt()
            refresh_day_menu()
    ...
    tk.Button(win, text="저장", command=save_verse).pack(pady=20)

def load_original_scriptures_txt():
    days = []
    fnames = []
    # data 폴더 내의 모든 txt 파일 검색
    data_dir = resource_path("data")
    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    # 파일명 정렬 (day1.txt, day2.txt ... 순서가 보장되도록 단순 정렬)
    # 필요하다면 숫자 추출 정렬 등을 적용할 수 있음
...
def refresh_day_menu():
    day_menu.delete(0, tk.END)
    for i, fname in enumerate(original_filenames):
        # 파일명에서 .txt 제거하고 표시
        label = fname.replace(".txt", "")
        day_menu.add_command(label=label, command=lambda idx=i: select_day(idx))
    
    day_menu.add_separator()
    day_menu.add_command(label="구절 추가", command=open_add_verse_popup)
    day_menu.add_separator()
