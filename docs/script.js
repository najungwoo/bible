        ...
        if (!isFullscreen) {
            if (reqFullscreen) {
                ...
                if (typeof customAlert === 'function') {
                    customAlert("아이폰(iOS) 브라우저에서는 전체화면 버튼 기능이 지원되지 않습니다.대신 하단의 [공유] ➔ [홈 화면에 추가]를 눌러 전체화면 앱처럼 사용해 보세요!");
                } else {
                    alert("아이폰(iOS) 브라우저에서는 전체화면 버튼 기능이 지원되지 않습니다.대신 하단의 [공유] ➔ [홈 화면에 추가]를 눌러 전체화면 앱처럼 사용해 보세요!");
                }
            }
        } else {
            if (exitFullscreen) {
